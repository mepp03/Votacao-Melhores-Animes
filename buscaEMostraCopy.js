...
const criaCardAnime = (
  ...
  const conteudo = `
        <img src="${large}" class="lista__item--img" id="${id}" data-nomeJ="${escaparString(
    romaji
  )}" data-nomeE="${escaparString(english)}" 
            onclick="passarEscolha('${id}', '${escaparString(
    romaji
  )}', '${escaparString(english)}', '${large}')">        
        <h3 class="lista__item--nome" id="nomeJ">${escaparString(romaji)}</h3>
        <h3 class="lista__item--nome" id="nomeE">${escaparString(english)}</h3>
        <a class="esconder" href="#">${escaparString(op)}</a>
        <p class="esconder">${escaparString(ed)}</p>
        `;
  // <a href="${video}" class="lista__item--link">${escaparString(op)}</a>
  //<a class="esconder" href="#" onClick="MyWindow=window.open('${video}','MyWindow','width=960,height=540'); return false;">${escaparString(op)}</a>
...
const criaCardAbertura = (id, op, video) => {
  ...
  const conteudo = `<video class="lista__item--video" id="video" width="720" height="480" controls>
        ...
        <h3 class="lista__item--nome" id="nomeOP">${escaparString(op)}</h3>
        <button class="lista__item--botao" id="${id}" data-nomeOp="${escaparString(
    op
  )}" onclick="passarEscolhaExtra('${escaparString(
    op
  )}')">Escolher</button>        
        `;
  // <a href="${video}" class="lista__item--link">${escaparString(op)}</a>
...
const criaCardEncerramento = (id, ed, video) => {
  ...
  const conteudo = `<video class="lista__item--video" id="video" width="720" height="480" controls>
        ...
        <h3 class="lista__item--nome" id="nomeED">${escaparString(ed)}</h3>
        <button class="lista__item--botao" id="${id}" data-nomeEd="${escaparString(
    ed
  )}" onclick="passarEscolhaExtra('${escaparString(
    ed
  )}')">Escolher</button>        
...
function procurar() {
  var nomeProcurado = document
    .getElementById("extra__busca")
