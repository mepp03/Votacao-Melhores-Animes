...
class EditorAnimesGUI:
    ...
    def adicionar_anime(self):
        ...
        dialog = AdicionarAnimeDialog(self.root)
        if dialog.resultado:
            novo_anime = dialog.resultado
    ...
    def editar_anime(self):
        ...
        dialog = EditarAnimeDialog(self.root, anime)
        if dialog.resultado:
            self.atualizar_lista_animes()
            self.dados[self.temporada_atual] = self.animes_atuais
    ...
    def atualizar_listas_op_ed(self):
        ...
        # Preencher openings
        openings = self.anime_selecionado.get('opening', {}).get('edges', [])
        for op in openings:
        ...
        # Preencher endings
        endings = self.anime_selecionado.get('ending', {}).get('edges', [])
        for ed in endings:
    ...
    def atualizar_preview(self):
        ...
        if self.anime_selecionado:
            ...
            preview_data = {
                ...
                'opening': self.anime_selecionado.get('opening'),
                'ending': self.anime_selecionado.get('ending')
            }
    ...
    def adicionar_opening(self):
        ...
        dialog = AdicionarOpEdDialog(self.root, "Opening")
        if dialog.resultado:
            nome, video = dialog.resultado
            novo_op = {
                'node': {
                    'op': {
                        'name': nome,
    ...
    def adicionar_ending(self):
        ...
        dialog = AdicionarOpEdDialog(self.root, "Ending")
        if dialog.resultado:
            nome, video = dialog.resultado
            novo_ed = {
                'node': {
    ...
    def editar_opening(self, event=None):
        ...
        dialog = AdicionarOpEdDialog(self.root, "Opening", nome_atual, video_atual)
        if dialog.resultado:
            novo_nome, novo_video = dialog.resultado
            opening['node']['op']['name'] = novo_nome
            opening['node']['op']['video'] = novo_video
            self.atualizar_listas_op_ed()
            self.dados[self.temporada_atual] = self.animes_atuais
    ...
    def editar_ending(self, event=None):
        ...
        dialog = AdicionarOpEdDialog(self.root, "Ending", nome_atual, video_atual)
        if dialog.resultado:
            novo_nome, novo_video = dialog.resultado
            ending['node']['ed']['name'] = novo_nome
            ending['node']['ed']['video'] = novo_video
            self.atualizar_listas_op_ed()
            self.dados[self.temporada_atual] = self.animes_atuais
...
# Diálogos (mantenha os mesmos da versão anterior)
class AdicionarAnimeDialog(simpledialog.Dialog):
    # ... (código igual ao anterior)

class EditarAnimeDialog(simpledialog.Dialog):
    # ... (código igual ao anterior)

class AdicionarOpEdDialog(simpledialog.Dialog):
    """Diálogo para adicionar/editar Opening ou Ending"""
    def __init__(self, parent, tipo, nome_atual="", video_atual=""):
        self.tipo = tipo
        self.nome_atual = nome_atual
        self.video_atual = video_atual
        self.resultado = None
        super().__init__(parent, title=f"Adicionar {tipo}")
    
    def body(self, frame):
        ttk.Label(frame, text=f"Nome do {self.tipo}:").grid(row=0, column=0, sticky=tk.W, pady=5)
        self.entry_nome = ttk.Entry(frame, width=40)
        self.entry_nome.insert(0, self.nome_atual)
        self.entry_nome.grid(row=0, column=1, sticky=(tk.W, tk.E), pady=5)
        
        ttk.Label(frame, text="URL do Vídeo:").grid(row=1, column=0, sticky=tk.W, pady=5)
        self.entry_video = ttk.Entry(frame, width=40)
        self.entry_video.insert(0, self.video_atual)
        self.entry_video.grid(row=1, column=1, sticky=(tk.W, tk.E), pady=5)
